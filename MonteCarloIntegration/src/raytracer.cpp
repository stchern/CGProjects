...
Color RayTracer::computeDirectLight(const ShadingIntersection& its, const Vector3D omegaO, bool pointLightsOnly) const
{
    const Material& material = scene.getInstances().at(its.instanceIndex).material;
    Color result{0.0f};
                                                         ...
                                                         Vector3D omegaO) const
{
    ...
    if (material.isConductor() || material.isRoughConductor()) {
        const Material::Conductor& conductor = material.isConductor() ?
                    material.conductor() : material.roughConductor().conductor;
...
void RayTracer::render()
{
    ...
    while (keep_rendering && sppRendered < params.maxSPP) {
        ...
        const Resolution numBlocks {(resolution.x+blockSize-1)/blockSize, (resolution.y+blockSize-1)/blockSize};
        ...
        for (--blockResDivider; blockSize>>blockResDivider; ++blockResDivider) {
            ...
            const uint32_t nextResIndex = numBlocks.x*numBlocks.y*(1U<<(2U*blockResDivider));
