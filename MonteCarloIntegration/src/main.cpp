...
public:
                          ...
                          /* gl_minor */ 1}
    {
        ...
        // setup the scene
        {
            ...
            // Materials
            const Material boxRed{Material::Diffuse{{0.8f, 0.4f, 0.3f}}};
            const Material boxGreen{Material::Diffuse{{0.4f, 0.8f, 0.3f}}};
            const Material boxWhite{Material::Diffuse{{0.7f, 0.7f, 0.7f}}};

            // Instances for scene
            scene.addInstance({"../meshes/CubeTopBackBottom.obj", boxWhite});
            scene.addInstance({"../meshes/CubeLeft.obj", boxRed});
            scene.addInstance({"../meshes/CubeRight.obj", boxGreen});

            // selection of Materials
            const Material diffuseGray{Material::Diffuse{{0.5f, 0.5f, 0.5f}}};
            const Material diffuseBlue{Material::Diffuse{{0.2f, 0.5f, 0.9f}}};
            const Material water{Material::Dielectric{1.33f, 1.0f}};
            const Material glass{Material::Dielectric{1.5f, 1.0f}};
            const Material gold{Material::Conductor{{0.143085f, 0.374852f, 1.44208f},
                                                    {3.98205f, 2.38506f, 1.60276f}}};
            const Material silver{Material::Conductor{{0.15522f, 0.116692f, 0.138342f},
                                                      {4.827f, 3.12139f, 2.14636f}}};
            const Material copper{Material::Conductor{{0.20038f, 0.923777f, 1.10191f},
                                                      {3.91185f, 2.45217f, 2.14159f}}};

            const Material emitter {diffuseBlue.diffuse(), diffuseBlue.diffuse().albedo*10.0f};

            const Material roughGold{Material::RoughConductor{
                Material::Conductor{{0.143085f, 0.374852f, 1.44208f},
                                    {3.98205f, 2.38506f, 1.60276f}},
                {0.1f}}};

            // selection of models...
            // TODO change materials if you wish
//             scene.addInstance({"../meshes/bunny.obj", gold,
//                               {Matrix3D::scale(5.f), {0.0f, -0.17f, -0.5f}}});
            scene.addInstance({"../meshes/gdv.obj", emitter, {{}, {0.0f, 0.5f, -1.0f}}});
