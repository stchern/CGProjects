...
void main() {
    ...
    vec3 srgb = vec3(
        value.x < 0.0031308 ? 12.92 * value.x : (1.055 * pow(value.x, 1.0/2.4) - 0.055),
        value.y < 0.0031308 ? 12.92 * value.y : (1.055 * pow(value.y, 1.0/2.4) - 0.055),
        value.z < 0.0031308 ? 12.92 * value.z : (1.055 * pow(value.z, 1.0/2.4) - 0.055)
    );
    ...
    : ImageView(parent), rayTracer{rayTracer}, cameraControls{cameraControls}
{
    ...
    auto renderMode = new ComboBox(rayTracerControls,
                                   {"Depth", "Position", "Normal", "Whitted", "Path"});
    renderMode->set_callback([&](int i) -> void { params.mode = RenderMode{i}; });
...
void RayTracerView::draw(NVGcontext* context)
{
    if (!texture || m_size != texture->size()) {
        ...
        progress->set_tooltip(std::to_string(static_cast<uint32_t>(sppRendered))+" / "s+std::to_string(params.maxSPP));
        m_image_shader->set_uniform("blur", 5.0f*std::exp(-sppRendered*(1.0f/std::log(5.0f))));
    }
