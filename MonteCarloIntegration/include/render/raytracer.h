...
public:
    ...
    Color depthIntegrator(const Scene& scene, const Ray& cameraRay) const;
    Color positionIntegrator(const Scene& scene, const Ray& cameraRay) const;
    Color normalIntegrator(const Scene& scene, const Ray& cameraRay) const;
    Color whittedIntegrator(const Scene& scene, const Ray& cameraRay, uint32_t depth = 0) const;
    Color pathIntegrator(const Scene& scene, const Ray& cameraRay) const;

    Color computeDirectLight(const ShadingIntersection& its, const Vector3D omegaO, bool pointLightsOnly=false) const;
