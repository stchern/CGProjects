...
void Mesh::loadOBJ(const std::string& filename)
{
    ...
    {
        std::vector<bool> usedVertexIndices(vertices.size());
        bool shadeFlat = true;
        ...
        for (size_t i = 0; i < faces.size(); ++i) {
            ...
            if (shadeFlat) {
                if (usedVertexIndices.at(faces.at(i).v3)) {
                    const uint32_t copyIndex = vertices.size();
                    vertices.push_back(vertices.at(faces.at(i).v3));
                    faces.at(i).v3 = copyIndex;
                    continue;
                }
            }
            usedVertexIndices.at(faces.at(i).v3) = true;
        }
    }
