...
/**
 * @brief Usually, a line is only represented by the indices of its vertices.
 */
struct LineIndices {
    uint32_t v1, v2;
};
...
/**
 * @brief Usually, a triangle is only represented by the indices of its vertices.
 */
...
/**
 * @brief The TriangleNormals struct references normals of a mesh to simplify working with a
 * concrete triangle.
 */
...
/**
 * @brief The BarycentricCoordinates struct contains two barycentric coordinates and computes the
 * third. It can be used to interpolate values within a triangle.
 */
...
public:
    ...
    Mesh(const std::string_view filename) { loadOBJ(filename); }
     ...
     * @param filename
     */
    void loadOBJ(const std::string_view filename);
